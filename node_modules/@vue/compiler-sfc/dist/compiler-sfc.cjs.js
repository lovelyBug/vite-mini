...
var scopedPlugin = postcss.plugin('vue-scoped', (options) => (root) => {
    const { id, vars: hasInjectedVars } = options;
...
const less = (source, map, options, load = require) => {
    ...
    const dependencies = getAbsolutePaths(result.imports, path.dirname(options.filename));
    if (map) {
        return {
            code: result.css.toString(),
...
const styl = (source, map, options, load = require) => {
    ...
    try {
        ...
        const dependencies = getAbsolutePaths(ref.deps(), path.dirname(options.fileName));
        if (map) {
            return {
                code: result,
...
function getAbsolutePaths(relativePaths, dirname) {
...
function compileScript(sfc, options = {}) {
    ...
    const plugins = [
        ...
        ...shared.babelParserDefautPlugins,
        ...(isTS ? ['typescript'] : [])
    ];
    if (!scriptSetup) {
        if (!script) {
