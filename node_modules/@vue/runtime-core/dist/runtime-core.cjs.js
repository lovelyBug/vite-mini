...
function normalizeEmitsOptions(comp) {
    ...
    if ( !shared.isFunction(comp)) {
        if (comp.extends) {
            hasExtends = true;
            shared.extend(normalized, normalizeEmitsOptions(comp.extends));
        }
        if (comp.mixins) {
            hasExtends = true;
            comp.mixins.forEach(m => shared.extend(normalized, normalizeEmitsOptions(m)));
        }
    }
    if (!raw && !hasExtends) {
...
function isEmitListener(comp, key) {
    let emits;
    if (!shared.isOn(key) || !(emits = normalizeEmitsOptions(comp))) {
        return false;
    }
    key = key.replace(/Once$/, '');
    return (shared.hasOwn(emits, key[2].toLowerCase() + key.slice(3)) ||
