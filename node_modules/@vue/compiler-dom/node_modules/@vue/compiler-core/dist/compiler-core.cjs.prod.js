...
const walkJS = (ast, walker) => {
    {
        return estreeWalker.walk(ast, walker);
    }
};
...
function createParserContext(content, options) {
    return {
        options: shared.extend({}, defaultParserOptions, options),
        column: 1,
        line: 1,
        offset: 0,
...
function hasCachedProps(node) {
    ...
    if (props && props.type === 15 /* JS_OBJECT_EXPRESSION */) {
        ...
        for (let i = 0; i < properties.length; i++) {
            ...
            if (val.type === 20 /* JS_CACHE_EXPRESSION */) {
                return true;
            }
            // merged event handlers
            if (val.type === 17 /* JS_ARRAY_EXPRESSION */ &&
                val.elements.some(e => !shared.isString(e) && e.type === 20 /* JS_CACHE_EXPRESSION */)) {
                return true;
            }
        }
    }
...
function createTransformContext(root, { prefixIdentifiers = false, hoistStatic = false, cacheHandlers = false, nodeTransforms = [], directiveTransforms = {}, transformHoist = null, isBuiltInComponent = shared.NOOP, expressionPlugins = [], scopeId = null, ssr = false, ssrCssVars = ``, bindingMetadata = {}, onError = defaultOnError }) {
    const context = {
        // options
        prefixIdentifiers,
        hoistStatic,
...
asRawStatements = false) {
    ...
    try {
        ast = parseJS(source, {
            plugins: [...context.expressionPlugins, ...shared.babelParserDefautPlugins]
        }).program;
    }
    catch (e) {
...
const transformModel = (dir, node, context) => {
    ...
    if (
        ...
        context.onError(createCompilerError(40 /* X_V_MODEL_ON_SCOPE_VARIABLE */, exp.loc));
        return createTransformProps();
    }
